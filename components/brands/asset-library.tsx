    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-[190px_minmax(0,1fr)]">
      ...
      <div className="min-w-0">
        <div className="grid gap-2 border bg-panel/25 p-2 sm:grid-cols-[minmax(160px,1fr)_auto_auto_auto_auto]">
          ...
          <select aria-label="Filter by status" value={status} onChange={(event) => setStatus(event.target.value as typeof status)} className="h-8 border bg-canvas px-2 text-[11px]">{statusOptions.map((item) => <option key={item}>{item}</option>)}</select>
...
function Preview({ asset, compact = false }: { asset: Asset; compact?: boolean }) {
  return <div className={cn("relative flex items-center justify-center overflow-hidden bg-elevated", compact ? "size-9 border" : "h-36 border-b")}>
    {asset.thumbnail ? <Image src={asset.thumbnail} alt="" fill sizes={compact ? "36px" : "320px"} className="object-contain p-3" /> : <File className="size-6 text-muted" />}
  </div>;
}
...
function AssetCard({ asset, selected, onSelect, onOpen, onPin }: { asset: Asset; selected: boolean; onSelect: (id: string, checked: boolean) => void; onOpen: () => void; onPin: () => void }) {
  return <article className={cn("relative border bg-panel/35", selected && "ring-2 ring-accent/60")}><label className="absolute left-2 top-2 z-10 flex size-6 items-center justify-center border bg-canvas"><span className="sr-only">Select {asset.name}</span><input type="checkbox" checked={selected} onChange={(event) => onSelect(asset.id, event.target.checked)} /></label><button onClick={onPin} aria-label={`${asset.pinned ? "Unpin" : "Pin"} ${asset.name}`} className="absolute right-2 top-2 z-10 flex size-6 items-center justify-center border bg-canvas"><Pin className={cn("size-3", asset.pinned && "fill-current text-accent")} /></button><button onClick={onOpen} className="w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent/50"><Preview asset={asset} /><div className="p-3"><div className="flex items-start justify-between gap-2"><h3 className="truncate text-xs font-medium">{asset.name}</h3><StatusBadge status={asset.status} /></div><p className="mt-2 text-[10px] text-muted">{asset.category} · {asset.extension}</p><p className="mt-1 text-[10px] text-muted">Updated {dateFormat.format(new Date(asset.updatedAt))}</p></div></button></article>;
}
