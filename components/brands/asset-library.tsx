...
import { Check, File, Folder, Grid2X2, List, Pin, Plus, Search, Trash2, Upload, X } from "lucide-react";
...
export function AssetLibrary({ brand }: { brand: Brand }) {
  const { assets, addAssets, togglePinned, updateAssets, deleteAssets } = useAssets();
  ...
  const confirmDelete = (ids: string[]) => {
    const targets = brandAssets.filter((asset) => ids.includes(asset.id));
    if (!targets.length) return;
    const connected = targets.filter((asset) => asset.category === "Logos" || asset.tags.includes("identity") || asset.pinned);
    const context = connected.length ? `\n\n${connected.length} selected Asset${connected.length === 1 ? " is" : "s are"} referenced by Identity or Guide. Those references will be removed immediately.` : "";
    if (!window.confirm(`Delete ${targets.length === 1 ? targets[0].originalFilename : `${targets.length} Assets`} from ${brand.name}?${context}\n\nThis affects this browser session only.`)) return;
    deleteAssets(targets.map((asset) => asset.id));
  ...
  return <section className="mt-6" aria-labelledby="assets-title">
    ...
    {detail ? <AssetDetail asset={detail} brandName={brand.name} onClose={() => setDetailId(null)} onPin={() => togglePinned(detail.id)} onStatus={(next) => updateAssets([detail.id], { status: next })} onDelete={() => confirmDelete([detail.id])} /> : null}
  </section>;
}
...
function AssetDetail({ asset, brandName, onClose, onPin, onStatus, onDelete }: { asset: Asset; brandName: string; onClose: () => void; onPin: () => void; onStatus: (status: AssetStatus) => void; onDelete: () => void }) {
  return <div className="fixed inset-0 z-[80] flex justify-end"><button className="absolute inset-0 bg-black/65" onClick={onClose} aria-label="Close Asset preview" /><aside role="dialog" aria-modal="true" aria-labelledby="asset-detail-title" className="relative z-10 h-full w-full max-w-md overflow-y-auto border-l bg-panel"><div className="flex items-center justify-between border-b p-4"><h3 id="asset-detail-title" className="text-sm font-medium">Asset details</h3><button onClick={onClose} aria-label="Close Asset details"><X className="size-4" /></button></div><Preview asset={asset} /><div className="space-y-5 p-5"><div><div className="flex items-start justify-between gap-3"><h4 className="text-base font-medium">{asset.name}</h4><StatusBadge status={asset.status} /></div><p className="mt-1 text-xs text-muted">{asset.originalFilename}</p><p className="mt-1 text-[10px] text-muted">{brandName}</p></div><dl className="grid grid-cols-2 gap-3 text-[11px]"><Meta label="Format" value={asset.extension} /><Meta label="Category" value={asset.category} /><Meta label="Dimensions" value={asset.width && asset.height ? `${asset.width} × ${asset.height}` : "—"} /><Meta label="Size" value={formatBytes(asset.sizeBytes)} /><Meta label="Version" value={asset.version ?? "—"} /><Meta label="Uploaded by" value={asset.uploadedBy.name} /><Meta label="Updated" value={dateFormat.format(new Date(asset.updatedAt))} /><Meta label="Tags" value={asset.tags.join(", ") || "—"} /></dl>{asset.description ? <div><p className="text-[10px] uppercase tracking-[0.1em] text-muted">Description</p><p className="mt-1 text-xs leading-5 text-subtle">{asset.description}</p></div> : null}{asset.usageNotes ? <div><p className="text-[10px] uppercase tracking-[0.1em] text-muted">Usage notes</p><p className="mt-1 text-xs leading-5 text-subtle">{asset.usageNotes}</p></div> : null}<div className="flex flex-wrap gap-2"><Button onClick={onPin}><Pin className="size-3.5" />{asset.pinned ? "Unpin" : "Pin"}</Button><select aria-label="Change Asset status" value={asset.status} onChange={(event) => onStatus(event.target.value as AssetStatus)} className="h-9 border bg-canvas px-2 text-xs">{statusOptions.slice(1).map((item) => <option key={item}>{item}</option>)}</select><Button variant="outline" onClick={() => {}}>Download unavailable</Button><Button variant="outline" onClick={onDelete} className="text-rose-300"><Trash2 className="size-3.5" />Delete Asset</Button></div></div></aside></div>;
}
