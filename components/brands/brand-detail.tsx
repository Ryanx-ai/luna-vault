...
import { ArrowLeft, Clock3, GitBranch, Settings2, Upload, UserRound, Users, X } from "lucide-react";
  ...
  return (
    <div className="mx-auto w-full max-w-[1120px] px-4 py-7 sm:px-6 sm:py-9 lg:px-8">
      ...
      <header className="relative flex flex-col gap-5 border-b pb-7 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2"><Button variant="outline" onClick={() => setHeaderPanel((panel) => panel === "manage" ? null : "manage")} aria-expanded={headerPanel === "manage"}><Settings2 className="size-3.5" />Manage Brand</Button><Button onClick={() => setHeaderPanel((panel) => panel === "upload" ? null : "upload")} aria-expanded={headerPanel === "upload"}><Upload className="size-3.5" />Upload Assets</Button></div>
        {headerPanel ? <div className="absolute right-0 top-full z-20 mt-2 w-[min(360px,calc(100vw-32px))] border bg-panel p-4 shadow-2xl"><div className="flex items-center justify-between"><p className="text-xs font-medium">{headerPanel === "manage" ? `Manage ${brand.name}` : `Upload Assets to ${brand.name}`}</p><button onClick={() => setHeaderPanel(null)} aria-label="Close Brand action" className="text-muted hover:text-foreground"><X className="size-3.5" /></button></div><p className="mt-2 text-[11px] leading-5 text-muted">{headerPanel === "manage" ? "Brand details, ownership, lifecycle, and archive controls will become editable in a later milestone." : `Uploads opened here will belong to ${brand.name}. Asset uploads remain deferred until Milestone 3; no files have been selected.`}</p></div> : null}
      </header>
