  ...
  return (
    <div className="mx-auto w-full max-w-[1180px] px-4 py-7 sm:px-6 sm:py-9 lg:px-8">
      <header className="flex flex-col gap-5 border-b pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-accent">{selectedVault.name} Vault</p><h1 className="mt-2 text-2xl font-semibold tracking-[-0.025em] sm:text-3xl">Brands</h1><p className="mt-2 max-w-xl text-xs leading-5 text-muted">Understand the identity systems, relationships, ownership, and rules inside this Vault.</p></div>
        <Button onClick={() => setCreateOpen(true)}><Plus className="size-3.5" />Create Brand</Button>
      </header>

      {vaultBrands.length === 0 ? (
        <section className="mt-6 border bg-panel/30"><EmptyState title={`No Brands in ${selectedVault.name}`} description="Create the first Brand to establish ownership, identity rules, and its place in the Brand Family." action={<Button onClick={() => setCreateOpen(true)}><Plus className="size-3.5" />Create Brand</Button>} /></section>
      ) : (
        <section className="mt-6" aria-labelledby="brand-family-heading">
          ...
          {view === "hierarchy" ? <BrandHierarchy brands={vaultBrands} /> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{vaultBrands.map((brand) => <BrandCard key={brand.id} brand={brand} parent={vaultBrands.find((candidate) => candidate.id === brand.parentBrandId)} />)}</div>}
        </section>
      )}

      <CreateBrandDialog open={createOpen} onClose={() => setCreateOpen(false)} vaultId={selectedVault.id} availableParents={parentBrands} />
    </div>
  );
}
