...
export function BrandsIndex() {
  ...
  const [pendingMove, setPendingMove] = useState<{ brandId: string; parentId: string } | null>(null);
    ...
    <div className="mx-auto w-full max-w-[1180px] px-4 py-7 sm:px-6 sm:py-9 lg:px-8">
      ...
      {vaultBrands.length === 0 ? (
        ...
        <section className="mt-6" aria-labelledby="brand-family-heading">
          ...
          {view === "hierarchy" ? <BrandHierarchy brands={vaultBrands} onRequestMove={(brandId, parentId) => setPendingMove({ brandId, parentId })} canMove={(brandId, parentId) => canMoveBrand(brandId, parentId).ok} /> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{vaultBrands.map((brand) => <BrandCard key={brand.id} brand={brand} />)}</div>}
        </section>
      )}

      <CreateBrandDialog open={createOpen} onClose={() => setCreateOpen(false)} vaultId={selectedVault.id} availableParents={eligibleParents} />
      {pendingMove ? <div className="fixed inset-0 z-[80] flex items-center justify-center p-4"><button className="absolute inset-0 bg-black/75" onClick={() => setPendingMove(null)} aria-label="Cancel Brand move" /><div role="dialog" aria-modal="true" aria-labelledby="move-brand-title" className="relative z-10 w-full max-w-sm border bg-panel p-5"><h2 id="move-brand-title" className="text-sm font-medium">Move {brands.find((item) => item.id === pendingMove.brandId)?.name} under {brands.find((item) => item.id === pendingMove.parentId)?.name}?</h2><p className="mt-2 text-xs text-muted">This hierarchy change lasts for this browser session.</p><div className="mt-5 flex justify-end gap-2"><Button variant="outline" onClick={() => setPendingMove(null)}>Cancel</Button><Button onClick={() => { moveBrand(pendingMove.brandId, pendingMove.parentId); setPendingMove(null); }}>Confirm</Button></div></div></div> : null}
    </div>
  );
}
