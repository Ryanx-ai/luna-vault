          ...
          <form onSubmit={submit} noValidate>
            <div className="space-y-4 p-5">
              ...
              {mode === "sub-brand" ? <>
                ...
                <div className="flex items-start justify-between gap-4 border p-3"><div><label htmlFor="inherit-parent-identity" className="text-xs font-medium text-subtle">Inherit parent identity</label><p className="mt-1 text-[10px] leading-4 text-muted">Use the parent Brand’s colours and typography.</p></div><button id="inherit-parent-identity" type="button" role="switch" aria-checked={inheritParentIdentity} aria-label={`Inherit parent identity: ${inheritParentIdentity ? "enabled" : "disabled"}`} onClick={() => setInheritParentIdentity((current) => !current)} className={`mt-0.5 inline-flex h-7 items-center gap-1.5 border px-2 text-[10px] font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50 ${inheritParentIdentity ? "border-accent/50 bg-accent/[0.09] text-accent shadow-[0_0_12px_rgba(183,244,242,0.08)]" : "border-rose-400/30 bg-rose-400/[0.06] text-rose-300"}`}>{inheritParentIdentity ? <Check className="size-3.5" /> : <X className="size-3.5" />}<span>{inheritParentIdentity ? "Enabled" : "Disabled"}</span></button></div>
              </> : null}
