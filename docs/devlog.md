...
## LV.TD.001-B — Final fixed shell correction

Founder clarification established that the complete top bar—breadcrumb, Search Vault, Updates, Notifications, and profile controls—must be outside the route scroller rather than sticky within it. The prior LV.TD.001 sticky implementation was therefore superseded. The main application column now uses a two-row `100dvh` grid with a fixed 56px top-bar row and a flexible `minmax(0, 1fr)` route viewport. Only the route `<main>` scrolls.

Concrete browser measurements on the long Kuro Brand page recorded route scroll changes of `0 → 638.5px` at 1440×900, `0 → 540px` at 1280×800, and `0 → 520px` at 390×844. Sidebar and top-bar tops remained `0`, breadcrumb top remained `19.5px`, route viewport top remained `56px`, and window/html/body scroll positions remained `0`. Expanded and collapsed desktop states, mobile drawer, top-bar panels, Vault switcher, Asset drawer, Create Brand dialog, requested routes, horizontal overflow, and browser console were also validated. `/brands/kurohotel` and its configure route currently render their honest unavailable states because Kuro Hotel is not present in the active fixture; long-page scrolling was proven on `/brands/kuro`. Milestone 4 has not started.
